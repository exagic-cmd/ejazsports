    ...
    <div class="row">
        <div class="col-12">
            <div class="content-header">
                <h2 class="content-title">Edit Receiving</h2>
                <div>
        ...
        <form action="{{route('receiving.store')}}" method="post" id="form" enctype="multipart/form-data" autocomplete="false" style="display: contents">
            @csrf
        <div class="col-lg-9">
            <div class="card mb-4">
                ...
                <div class="card-body">
                        ...
                        <div @class('row')>
                            ...
                            <div class="col-lg-6">
                                <div class="mb-4">
                                    ...
                                    <input  type="date" id="date" name="date" class="form-control @error('date') is-invalid @enderror" value="{{$receiving->date}}" >
                                    @error('date')
                                    <div class="alert alert-danger">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <div class="col-lg-6">
                            <div class="mb-4">
                                ...
                                <input  type="text" name="cargo_no" class="form-control @error('cargo_no') is-invalid @enderror" value="{{$receiving->cargo_no}}" >
                                @error('cargo_no')
                                <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
        ...
        <div class="col-lg-12">
            ...
            <tr id="r{{$p->id}}" style="background-color: #ffdada;">
                <td>#</td>
                ...
                <td>
                    <select @class('form-control select2') name="product[]">
                            ...
                            <option value="{{$p->product->id}}"><b>{{$p->product->title}}</b></option>
                ...
                <td>
                     ...
                     <select @class('form-control select2') name="variant[]">
                          ...
                          @if($p->variant)
                            ...
                            @foreach($p->product->variants as $v)
                            @if($p->variant->id == $v->id)
                                <option value="{{$v->id}}" selected=""> {{$v->shade}} - {{$v->size}}</option>
                                @else
                ...
                <td><input type="number" @class('form-control') onkeyup="updateDiff({{$p->id}})" name="r_qty[]" id="r_qty{{$p->id}}" value="{{$p->qty}}">
                           
                        </td>
                        
                        
                         <td><button class="form-control btn btn-primary" type="button" onclick="addQuantity({{$p->id}})" >Add</button>
                            <br> <span>Received : <span id="add_qty{{$p->id}}" >0</span></span><br>
                            <span id="text_qty{{$p->id}}" >0</span>
                        </td>

                <input type="hidden" value="0" name="po_product[]">
                <input type="hidden" value="{{$p->qty}}" @class('form-control qty') name="received_qty[]" id="t_r_qty{{$p->id}}">
            </tr>
                         @endforeach   
           ...
           <input type="hidden" value="{{$receiving->id}}" name="rr_id">
        </form>
