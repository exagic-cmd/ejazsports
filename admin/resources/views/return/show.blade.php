...
@section('content')
    ...
    <div class="card mycustom-card">
        ...
        <div class="purchase-order-main">
            <div><img width="100px" src="{{ asset('imgs/theme/logo.jpeg') }}" alt="logo"></div>
            <div>
                <h2>Return Notes</h2>
            </div>
        ...
        <div class="vendor-main">
            ...
            <div>
                <h3>Vendor</h3>
                <p><b>{{ strtoupper($receiving->purchaseOrder?->supplier?->name ?? ($receiving->supplier?->name ?? '')) }}</b>
                </p>
                <p>Phone:
                    {{ ($receiving->purchaseOrder ? ($receiving->purchaseOrder->supplier ? $receiving->purchaseOrder->supplier->mobile_number : '') : $receiving->supplier) ? $receiving->supplier->mobile_number : '' }}
                </p>
                <p>Email:
                    {{ ($receiving->purchaseOrder ? ($receiving->purchaseOrder->supplier ? $receiving->purchaseOrder->supplier->email : '') : $receiving->supplier) ? $receiving->supplier->email : '' }}
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

                </p>
                <br><br>
                <p> <b>Payment Method : </b>
                    @if ($receiving->payment_method == \App\Models\SupplierBrand::CASH)
                        CASH
                ...
                <div class="d-print-none">
                    <br>
                    <p><b>Stock: </b>{{ $receiving->stock_type == 2 ? 'Damage Stock' : 'Fresh Stock' }}</p>
                ...
                <table class="fl-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><b style=" border-bottom: 4px double;">{{ number_format($receiving->gross_amount) }}</b>
                            </td>
        ...
        <div class="purchase-total">
            <div style="    min-width: 40%;border: 1px solid;padding: 5px;">
                <p>{{ $receiving->comment }}</p>
            </div>
            <div>
    ...
    <div @class('d-flex')>
        ...
        @if ($receiving->status != \App\Models\Receiving::APPROVAL_PENDING)
            <div class="download-print-btn text-center" style="margin-bottom: 40px;    margin-left: 30%;">
                ...
                <a href="{{ route('receiving.grn', $receiving->id) }}" target="_blank"
                    class="btn btn-primary download-btn">GRN</a>
            </div>
        @else
            <div class="download-print-btn text-center" style="margin-bottom: 40px;    margin-left: 30%;">
        ...
        function changeStatus(receiving_id, status) {
            ...
            $.confirm({
                ...
                buttons: {
                    confirm: function() {
                        $.ajax({
                            url: "{{ route('supplier-returns.change.status') }}",
                            type: 'GET',
                            data: {
