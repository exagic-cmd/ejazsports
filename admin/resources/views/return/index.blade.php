...
@section('content')
    ...
    <div class="content-header">
        <div>
            <h2 class="content-title card-title">All Supplier Returns</h2>
            <p>Receiving information.</p>
        </div>
        @can('Create Receiving')
            <div>
                <a  href="{{route('supplier-returns.create')}}" class="btn btn-primary"><i class="text-muted material-icons md-post_add"></i>Add New</a>
            </div>
        @endcan
                    ...
                    @foreach($receiving as $r)
                        ...
                        <tr >
                            ...
                            <td class="text-end">
                                ...
                                @can('View Receiving')
                                    <a href="{{route('supplier-returns.show',$r->id)}}" class="btn btn-md rounded font-sm">Detail</a>
                                @endcan
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            @can('Edit Receiving')
                                                <a class="dropdown-item" href="{{route('supplier-returns.edit',$r->id)}}">Edit info</a>
                                            @endcan

                                            @can('Delete Receiving')
                                                <form onsubmit="return confirm('Do you really want to do this?');" id="delete-form" action="{{ route('supplier-returns.destroy',$r->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
