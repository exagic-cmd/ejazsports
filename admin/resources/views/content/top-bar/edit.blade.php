    ...
    <div class="row">
        <div class="col-9">
            <div class="content-header">
                <h2 class="content-title">Edit Top Bar Content</h2>
                <div>
        ...
        <div class="col-lg-6">
            <form action="{{route('top-bar.update',$topBarContent->id)}}" method="post" id="form" autocomplete="false" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-4">
                            ...
                            <input  type="text" name="text" placeholder="Type here" class="form-control @error('text') is-invalid @enderror" value="{{ $topBarContent->text }}">
                            @error('text')
                        ...
                        <div class="mb-4 tooltip1">
                            ...
                            <input  type="number" name="serial_no" placeholder="Type here" class="form-control @error('serial_no') is-invalid @enderror" value="{{ $topBarContent->serial_no }}">
                            @error('serial_no')
                            <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input class="form-check-input" value="1" type="checkbox" {{$topBarContent->web_active ? 'checked' : ''}} name="web_active" id="flexSwitchCheckChecked" >
                            @error('show_in_menu')
                            <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input class="form-check-input" value="1" type="checkbox" {{$topBarContent->mobile_active ? 'checked' : ''}} name="mobile_active" id="flexSwitchCheckChecked" >
                            @error('mobile_active')
                            <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="status" class="form-control select2" >
                                <option value="1" {{$topBarContent->status ? 'selected' : ''}}>Active</option>
                                <option  value="0" {{(!$topBarContent->status) ? 'selected' : ''}}>InActive</option>
                            </select>
                            @error('status')
                            <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
...
@section('js')
    <script>
        ...
        $('body').on("keyup", "input[name=title]", function() {
            ...
            $('input[name="category_heading"]').val($('input[name="title"]').val());
