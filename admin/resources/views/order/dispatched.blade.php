...
@section('content')
    ...
    <div class="content-header">
        <div>
            <h2 class="content-title card-title">All Dispatched Orders</h2>
            <p>Order Information {{number_format($orders->sum('total_amount'))}} - ({{count($orders)}})</p>
        </div>
        @can('Dispatch Orders')
            <div>
                <a href="{{route('orders.dispatch.new')}}" class="btn btn-primary"><i class="text-muted material-icons md-post_add"></i>Add New Dispatch</a>
            </div>
        @endcan
                ...
                <table id="myTable" class="table table-hover">
                    ...
                    @foreach($orders as $order)
                        ...
                        <tr style="text-align: center;">
                            ...
                            <td>
                                {{date('d M, Y h:i',strtotime($order->dispatch_time))}}
                            </td>
