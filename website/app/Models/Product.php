...
class Product extends Model
{
    ...
    public function store($request) {
        ...
        try {
            ...
            if($request->color_id && $request->size_id) {
                foreach ($request->color_id as $key => $color) {
                    ...
                    foreach ($request->size_id as $key => $size) {
                        ...
                        $productVariant->barcode = time() . $count++;
            ...
            if($request->color_id || $request->size_id)
                        Product::where('id',$product->id)->update(['have_variants' => true,'barcode'=>null]);
            ...
            if($request->barcode) {
                ...
                for($i =0;$i < count($request->barcode); $i++) {
                    ...
                    if($request->variant_id[$i] != 0)
                        ...
                        $productVariant = new ProductVariant();
                    $productVariant->product_id = $product->id;
                    $productVariant->barcode = ($request->barcode[$i]) ? $request->barcode[$i] : time() . $count++;
